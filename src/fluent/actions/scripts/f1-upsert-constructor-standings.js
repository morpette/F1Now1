/**
 * Parses a f1api.dev /api/{year}/constructors-championship-style JSON body
 * ({ championshipId, constructors_championship: [{ teamId, position,
 * points, wins, ... }] }) and upserts each entry into
 * x_1912467_f1now1_team_standing, matched on the (championship, team)
 * unique index. Entries whose championship or team can't be resolved by
 * business id are skipped, not errored.
 */
(function execute(inputs, outputs) {
    ...
    var skipped = 0;
    try {
        var parsed = JSON.parse(inputs.constructor_standings_json);
        var championshipId = parsed && parsed.championshipId;
        var standings = (parsed && parsed.constructors_championship) || [];
        ...
        for (var i = 0; i < standings.length; i++) {
            ...
            var gr = new GlideRecord('x_1912467_f1now1_team_standing');
