...
export const f1SearchDrivers = Action(
    {
        ...
        masterSnapshot: '4533d738937dcf14a03577f08bba1069',
    },
    (params) => {
        const invoke_f1_api_searchdrivers = wfa.actionStep(
            actionStep.script,
        ...
        wfa.assignActionOutputs(params.outputs, {
            response_body: wfa.dataPill(invoke_f1_api_searchdrivers.response_body, 'string'),
            status_code: wfa.dataPill(invoke_f1_api_searchdrivers.status_code, 'integer'),
            error_message: wfa.dataPill(invoke_f1_api_searchdrivers.error_message, 'string'),
        })
    }
)
